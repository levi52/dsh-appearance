...
const FONT_SIZES: Record<string, string> = { small: "13px", normal: "14px", large: "16px" };
...
export function apply(ctx: ClientContext): void {
	...
	ctx.effect(() => () => { for (const dispose of disposers) dispose(); }, "dsh-appearance: theme registry");
	...
	function applyFonts(value: AppearanceSettings | undefined): void {
		...
		if (size) root.style.setProperty("font-size", size);
	...
	ctx.on("theme/change", (snapshot) => {
		...
		const pref = (snapshot as ThemeSnapshot).preference;
		if ((BUILTIN as readonly string[]).indexOf(pref) !== -1 && currentTheme() !== pref) scope.set("theme", pref);
	});
