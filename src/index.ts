...
/** Minimal node:http handler surface used by this plugin. */
interface HttpHandlerContext {
	webServer: {
		register(route: { path: string; handler: (req: unknown, res: unknown) => void | Promise<void> }): unknown;
	};
}
...
export function apply(ctx: HostContext): void {
	...
	ctx.inject(["webServer"], (httpCtx) => {
		const server = (httpCtx as unknown as HttpHandlerContext).webServer;
		httpCtx.effect(() => {
			server.register({
